...
- Telekasten/Markdown UI: новые оттенки в палитре и отдельный модуль для Telekasten перекрашивают иконки, теги и wiki-скобки в глубокий холодный угольный цвет, так что счётчики и элементы панели становятся заметнее, при этом diff-цвета не трогаются.
- Рамп цветов заголовков Markdown теперь комбинирует более яркий `accent_secondary` (для уровней 1–3) и классический include-синий (для уровней 4–6), что заметно повышает контраст без необходимости менять `cfg.markup.headings`.
...
- Git-палитра: все `diff*`/`GitSigns*`/диагностики получили новую холодную связку (тёмная ель для добавлений, холодный индиго для изменений, тёмный бордо для удалений) — насыщенно, но без кислотности, в духе остальной палитры.
- Единый диспетчер `:Neg`: новая схема опций и команда `:Neg {set|toggle|info|scenario|plugins|export|import}` с многоуровневым автодополнением (подкоманда → путь → значения) заменяет россыпь `:Neg*` тумблеров. Старые команды сохранились как тонкие обёртки и работают через тот же движок.
