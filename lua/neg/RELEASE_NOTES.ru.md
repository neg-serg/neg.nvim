...
- Git-палитра: все `diff*`/`GitSigns*`/диагностики получили новую связку (мятный зелёный для добавлений, янтарный для изменений, коралловый для удалений), чтобы значки и подсветка состояний читались одинаково в знаках, гуттерах и виртуальном тексте.
